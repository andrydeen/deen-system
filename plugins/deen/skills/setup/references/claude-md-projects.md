...
Every action should trace back to an objective:

```
Goals / OKRs -> Projects -> Monthly Focus -> Weekly Review -> Daily Tasks
```

- Goals and OKRs live in `Context/strategy.md`
- Projects in `Projects/` link to those goals
- During weekly reviews, check which goals have no active project (they're drifting)
- During quarterly reviews, assess goal progress and cascade updates
